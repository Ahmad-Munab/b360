...
import { subscriptionUsage, subscription, widget, widgetAnalytics, user } from '@/db/schema';
...
export class UsageService {
     ...
     */
    static async initializeUsageRecord(userId: string, period?: string): Promise<void> {
        ...
        if (!userExistsInDb) {
            ...
            console.warn(`User ${userId} not found in database, skipping usage record creation`);
            return;
        }
     ...
     */
    static async incrementMessageUsage(userId: string, widgetId: string, count: number = 1): Promise<boolean> {
    ...
    /**
     * Increment feedback count for a widget
     */
    static async incrementFeedbackCount(widgetId: string): Promise<boolean> {
        try {
            const existingWidgetAnalytics = await db
                .select()
                .from(widgetAnalytics)
    ...
    /**
     * Increment bug report count for a widget
     */
    static async incrementBugReportCount(widgetId: string): Promise<boolean> {
        try {
            const existingWidgetAnalytics = await db
                .select()
                .from(widgetAnalytics)
            ...
            console.error('Error incrementing bug report count:', error);
            return false;
        }
    }
     ...
     */
    static async canPerformAction(userId: string, action: 'message' | 'widget'): Promise<boolean> {
    ...
    static async getUserUsageHistory(userId: string) {
        ...
        const usageHistory = await db
            ...
            .where(
                and(
                    eq(subscriptionUsage.userId, userId),
                    sql`${subscriptionUsage.period} IN (${periods.map(p => `'${p}'`).join(',')})`
                )
            )
